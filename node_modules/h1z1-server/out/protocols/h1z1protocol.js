...
class H1Z1Protocol {
    H1Z1Packets;
    protocolName;
    PlayerUpdateManagedPositionOpcode;
    VehicleCollisionOpcode;
    VehicleDimissOpcode;
    weaponOpcode;
    ...
    createPositionBroadcast2016(rawData, transientId) {
        const transientIdPacked = (0, shared_1.packUnsignedIntWith2bitLengthValue)(transientId);
        const buff = Buffer.allocUnsafe(1 + transientIdPacked.length + rawData.length);
        buff[0] = 0x79; // opcode
        transientIdPacked.copy(buff, 1);
        rawData.copy(buff, 1 + transientIdPacked.length);
        return buff;
    }
    createManagedPositionBroadcast2016(rawData) {
    ...
    parsePlayerUpdatePosition(data, offset) {
        return {
    ...
    pack(packetName, object = {}) {
        ...
        if (packet) {
            ...
            if (packet.schema) {
                try {
                    packetData = h1z1_dataschema_1.default.pack(packet.schema, object);
                }
                catch (error) {
    ...
    parse(data, flag) {
        ...
        switch (flag) {
            case h1emu_core_1.GatewayChannels.World:
            case h1emu_core_1.GatewayChannels.Zone: {
                {
                    [packet, offset] = this.resolveOpcode(opCode, data);
                    break;
                }
            }
            case h1emu_core_1.GatewayChannels.UpdatePosition: {
                try {
                    packet = {
            ...
            case h1emu_core_1.GatewayChannels.ShortCircuitZone: {
                switch (opCode) {
