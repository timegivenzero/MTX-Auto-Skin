...
process.on("unhandledRejection", (reason, promise) => {
    console.error("Unhandled rejection at ", promise, `reason: ${reason} at ${new Date()}`);
    logVersion();
    process.exit(1);
});
...
process.on("SIGTERM", () => {
    console.error(`Process ${process.pid} received a SIGTERM signal time : ${new Date()}`);
    logVersion();
    process.exit(0);
});
process.on("SIGINT", () => {
    console.error(`Process ${process.pid} has been interrupted time : ${new Date()}`);
    logVersion();
    process.exit(1);
});
