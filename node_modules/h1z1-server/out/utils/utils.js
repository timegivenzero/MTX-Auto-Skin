...
exports.TimeWrapper = exports.getRandomKeyFromAnObject = exports.getRandomFromArray = exports.toHex = exports.toBigHex = exports.wrappedUint16 = exports.clearFolderCache = exports.getPacketTypeBytes = exports.initMongo = exports.lz4_decompress = exports.LZ4 = exports.generateCommandList = exports.removeCacheFullDir = exports.generateRandomGuid = exports.Int64String = exports.toInt = exports.isPosInRadiusWithY = exports.isPosInRadius = exports.isInsideCube = exports.isInsideSquare = exports.objectIsEmpty = exports.setupAppDataFolder = exports.getAppDataFolderPath = exports.randomIntFromInterval = exports._ = exports.customLodash = void 0;
exports.isQuat = isQuat;
exports.eul2quat = eul2quat;
exports.quat2matrix = quat2matrix;
exports.eul2quatLegacy = eul2quatLegacy;
exports.fixEulerOrder = fixEulerOrder;
exports.movePoint = movePoint;
exports.isPointNearLine = isPointNearLine;
exports.getAngle = getAngle;
exports.rotateAroundPivot = rotateAroundPivot;
exports.checkLineThroughDoorway = checkLineThroughDoorway;
exports.wallInterceptsLine = wallInterceptsLine;
exports.zoneShutdown = zoneShutdown;
exports.getDifference = getDifference;
exports.decrypt = decrypt;
exports.getDistance = getDistance;
exports.getDistance2d = getDistance2d;
exports.getDistance1d = getDistance1d;
...
exports.getCubeBounds = getCubeBounds;
exports.generateTransientId = generateTransientId;
exports.calculate_falloff = calculate_falloff;
...
exports.removeUntransferableFields = removeUntransferableFields;
exports.isFloat = isFloat;
exports.fileExists = fileExists;
exports.copyFile = copyFile;
exports.getCurrentServerTimeWrapper = getCurrentServerTimeWrapper;
exports.getCurrentRealTimeWrapper = getCurrentRealTimeWrapper;
exports.getDateString = getDateString;
exports.loadJson = loadJson;
exports.loadNavData = loadNavData;
exports.isHalloween = isHalloween;
exports.isChristmasSeason = isChristmasSeason;
exports.luck = luck;
exports.isPosInPoi = isPosInPoi;
exports.isLootNerfedLoc = isLootNerfedLoc;
exports.chance = chance;
exports.getCellName = getCellName;
exports.quat2heading = quat2heading;
// ======================================================================
//
...
const constructiondoor_1 = require("../servers/ZoneServer2016/entities/constructiondoor");
 ...
 */
function fixEulerOrder(rotation) {
...
function isPointNearLine(point, lineStart, lineEnd, threshold) {
    ...
    const projection = (pointVector[0] * lineVector[0] + pointVector[1] * lineVector[1]) /
    ...
    const distance = Math.sqrt((point[0] - closestPoint[0]) * (point[0] - closestPoint[0]) +
 ...
 */
function rotateAroundPivot(pivot, point, angle) {
 ...
 */
function checkLineThroughDoorway(a1, a2, doorway) {
    const door = doorway.occupiedWallSlots[1];
    if (door && door instanceof constructiondoor_1.ConstructionDoor && !door.isOpen)
        return false;
    let doorEnd;
    let doorStart;
    if (door instanceof constructiondoor_1.ConstructionDoor && door.isOpen) {
        ...
        doorStart = new Float32Array(doorway.getSlotPosition(1, doorway.wallSlots));
 ...
 */
function wallInterceptsLine(a1, a2, b1, b2) {
    b1[1] -= 0.5; // extend the plane down a bit
    const orient = (p, q, r) => (q[0] - p[0]) * (r[2] - p[2]) - (q[2] - p[2]) * (r[0] - p[0]);
...
const setupAppDataFolder = () => {
    ...
    if (!node_fs_1.default.existsSync(AppDataFolderPath)) {
        node_fs_1.default.mkdirSync(AppDataFolderPath);
    }
    if (!node_fs_1.default.existsSync(`${AppDataFolderPath}/single_player_characters.json`)) {
        node_fs_1.default.writeFileSync(`${AppDataFolderPath}/single_player_characters.json`, JSON.stringify([]));
    }
    if (!node_fs_1.default.existsSync(`${AppDataFolderPath}/single_player_accountitems.json`)) {
        node_fs_1.default.writeFileSync(`${AppDataFolderPath}/single_player_accountitems.json`, JSON.stringify([]));
    }
    if (!node_fs_1.default.existsSync(`${AppDataFolderPath}/single_player_characters2016.json`) ||
        node_fs_1.default
            .readFileSync(`${AppDataFolderPath}/single_player_characters2016.json`)
            .toString() === "{}") {
        node_fs_1.default.writeFileSync(`${AppDataFolderPath}/single_player_characters2016.json`, JSON.stringify([]));
    }
    if (!node_fs_1.default.existsSync(`${AppDataFolderPath}/single_player_charactersKOTK.json`)) {
        node_fs_1.default.writeFileSync(`${AppDataFolderPath}/single_player_charactersKOTK.json`, JSON.stringify([]));
    }
    if (!node_fs_1.default.existsSync(`${AppDataFolderPath}/worlddata`)) {
        node_fs_1.default.mkdirSync(`${AppDataFolderPath}/worlddata`);
    }
    if (!node_fs_1.default.existsSync(`${AppDataFolderPath}/worlddata/vehicles.json`)) {
        node_fs_1.default.writeFileSync(`${AppDataFolderPath}/worlddata/vehicles.json`, JSON.stringify([]));
    }
    if (!node_fs_1.default.existsSync(`${AppDataFolderPath}/worlddata/construction.json`)) {
        node_fs_1.default.writeFileSync(`${AppDataFolderPath}/worlddata/construction.json`, JSON.stringify([]));
    }
    if (!node_fs_1.default.existsSync(`${AppDataFolderPath}/worlddata/worldconstruction.json`)) {
        node_fs_1.default.writeFileSync(`${AppDataFolderPath}/worlddata/worldconstruction.json`, JSON.stringify([]));
    }
    if (!node_fs_1.default.existsSync(`${AppDataFolderPath}/worlddata/crops.json`)) {
        node_fs_1.default.writeFileSync(`${AppDataFolderPath}/worlddata/crops.json`, JSON.stringify([]));
    }
    if (!node_fs_1.default.existsSync(`${AppDataFolderPath}/worlddata/traps.json`)) {
        node_fs_1.default.writeFileSync(`${AppDataFolderPath}/worlddata/traps.json`, JSON.stringify([]));
    }
    if (!node_fs_1.default.existsSync(`${AppDataFolderPath}/worlddata/world.json`)) {
        node_fs_1.default.writeFileSync(`${AppDataFolderPath}/worlddata/world.json`, JSON.stringify({}));
    }
};
 ...
 */
const isInsideCube = (point, bounds) => {
    const x = point[0], z = point[2], lowerY = bounds[0][1], upperY = bounds[7][1];
    let inside = false;
    for (let i = 0, j = 4 - 1; i < 4; j = i++) {
        const xi = bounds[i][0], zi = bounds[i][2], xj = bounds[j][0], zj = bounds[j][2], intersect = zi > z != zj > z && x < ((xj - xi) * (z - zi)) / (zj - zi) + xi;
        if (intersect)
            inside = !inside;
    }
 ...
 */
const isPosInRadius = (radius, position1, position2) => {
 ...
 */
const isPosInRadiusWithY = (radius, position1, position2, y_radius) => {
    return (isBetween(radius, position1[0], position2[0]) &&
 ...
 */
function getDistance1d(height1, height2) {
 ...
 */
function getCubeBounds(centerPoint, angle, offset, eulerRot, bottomY, topY) {
    const middlePointA = movePoint(centerPoint, eulerRot, offset / 2);
    const middlePointB = movePoint(centerPoint, eulerRot + (180 * Math.PI) / 180, offset / 2);
    const pointA = movePoint(middlePointA, eulerRot + 90 * (Math.PI / 180), angle / 2);
    const pointB = movePoint(middlePointA, eulerRot + 270 * (Math.PI / 180), angle / 2);
...
function* generateTransientId() {
    for (let index = 1; index < constants_1.MAX_TRANSIENT_ID; index++) {
        yield index;
    }
}
/**
...
const removeCacheFullDir = function (directoryPath) {
    const files = (0, node_fs_1.readdirSync)(directoryPath); // need to be sync
    for (const file of files) {
        ...
        if (file.substring(file.length - 3) === ".js") {
            delete require.cache[(0, node_path_1.normalize)(`${directoryPath}/${file}`)];
        }
    }
};
...
const initMongo = async function (mongoClient, serverName) {
    ...
    await mongoClient
        ...
        .insertMany(servers);
    await mongoClient.db(dbName).createCollection(enums_1.DB_COLLECTIONS.ADMINS);
    const admins = require("../../data/defaultDatabase/shared/admins.json");
    await mongoClient
        .db(dbName)
        .collection(enums_1.DB_COLLECTIONS.ADMINS)
        .insertMany(admins);
...
const clearFolderCache = (currentFolderDirname, folderPath) => {
    const resolvedPath = (0, node_path_1.resolve)(currentFolderDirname, folderPath);
 ...
 */
function calculate_falloff(distance, minDamage, maxDamage, falloffStart, falloffEnd) {
    ...
    const damageRange = maxDamage - minDamage, distanceRange = falloffEnd - falloffStart, distanceFromStart = distance - falloffStart, interpolation = 1 - distanceFromStart / distanceRange, reducedDamage = minDamage + interpolation * damageRange;
...
function isValidCharacterName(name) {
    try {
        ...
        const hasSpecialChars = specialCharRegex.test(name) || regexSpecialChars.test(name);
        ...
        console.error(e);
        return enums_1.NAME_VALIDATION_STATUS.INVALID;
    }
}
/**
 ...
 */
function removeUntransferableFields(data) {
...
function fileExists(filePath) {
    try {
        node_fs_1.default.accessSync(filePath);
        return true;
    }
    catch (error) {
        console.error(error);
...
async function copyFile(originalFilePath, newFilePath) {
    return new Promise((resolve, reject) => {
        const readStream = node_fs_1.default.createReadStream(originalFilePath);
        const writeStream = node_fs_1.default.createWriteStream(newFilePath);
...
class TimeWrapper {
    fullTimeMs;
    constructor(fullTimeMs) {
        this.fullTimeMs = fullTimeMs;
    }
    getSeconds() {
        return (0, exports.toInt)(this.fullTimeMs / 1000);
    }
    getMinutes() {
        return (0, exports.toInt)(this.fullTimeMs / 60000);
    }
    getHours() {
        return (0, exports.toInt)(this.fullTimeMs / 3600000);
    }
    getFull() {
    ...
    getFullString() {
        return (0, exports.Int64String)(this.fullTimeMs);
    }
    getTruncatedU32() {
        const truncated = this.fullTimeMs & constants_1.MAX_UINT32;
        return truncated >= 0 ? truncated : truncated >>> 0;
    }
    getTruncatedU32String() {
        const truncated = this.fullTimeMs & constants_1.MAX_UINT32;
        return truncated >= 0
            ? (0, exports.Int64String)(truncated)
            : (0, exports.Int64String)(truncated >>> 0);
    }
}
exports.TimeWrapper = TimeWrapper;
function getCurrentServerTimeWrapper() {
    return new TimeWrapper(Date.now() - startTime);
}
function getCurrentRealTimeWrapper() {
...
function loadNavData() {
    ...
    files.forEach((file) => {
        ...
        if (stats.isFile()) {
            ...
            const fileIndex = Number(filePath.split(".")[0].split("_part")[1]) - 1;
    ...
    Z1_POIs.forEach((point) => {
        ...
        if (point.bounds) {
            ...
            point.bounds.forEach((bound) => {
                if ((0, exports.isInsideSquare)([position[0], position[2]], bound)) {
                    isInPoi = true;
                    return;
                }
            });
        }
        if (useRange && (0, exports.isPosInRadius)(point.range, position, point.position)) {
            isInPoi = true;
        }
    });
    return isInPoi;
...
function isLootNerfedLoc(position) {
    let useRange = true;
    let nerfedValue = 0;
    Z1_nerfedPOIs.forEach((point) => {
        if (point.bounds) {
            useRange = false;
            point.bounds.forEach((bound) => {
                if ((0, exports.isInsideSquare)([position[0], position[2]], bound)) {
...
function chance(chanceNum) {
    return Math.random() * 1000 < chanceNum;
}
function getCellName(index, numCols) {
...
function quat2heading(quaternion) {
