...
const emotes_1 = require("../../data/emotes");
...
exports.commands = [
    ...
    },
    {
        ...
        execute: async (server, client, args) => {
            if (server._soloMode)
                return;
            const collection = server._db.collection(enums_2.DB_COLLECTIONS.KILLS);
    ...
    },
    {
        ...
        execute: (server, client, args) => {
            if (server.isBattleRoyale())
                return;
            const position = server.calculatePosFromSpawnCell(server._spawnGrid[(0, utils_1.randomIntFromInterval)(0, 99)]);
    ...
    },
    {
        ...
        permissionLevel: types_1.PermissionLevels.DEFAULT,
        execute: (server, client, args) => {
            server.sendChatText(client, `Spawned entities count : ${client.spawnedEntities.size}`);
        }
    },
    {
        name: "serverinfo",
        ...
        execute: (server, client, args) => {
            if (args[0] === "mem") {
                ...
                const { _clients: clients, _npcs: npcs, _spawnedItems: objects, _vehicles: vehicles, _destroyables: dto } = server;
                ...
                if (client.isAdmin) {
                    server.sendChatText(client, `clients : ${utils_1._.size(clients)} | npcs : ${utils_1._.size(npcs)}`);
                }
                server.sendChatText(client, `items : ${utils_1._.size(objects)} | vehicles : ${utils_1._.size(vehicles)}`);
                server.sendChatText(client, `dto: ${utils_1._.size(dto)}`);
            }
        }
    },
    {
        ...
        permissionLevel: types_1.PermissionLevels.ADMIN,
        execute: (server, client, args) => {
            const pop = utils_1._.size(server._clients);
            server.sendChatText(client, `There ${pop > 1 ? "are" : "is"} ${pop} player${pop > 1 ? "s" : ""} online.`);
        }
    },
    {
        name: "spawninfo",
    ...
    },
    {
        ...
        execute: (server, client, args) => {
            ...
            for (const a in server._clients) {
                ...
                c.clientLogs.forEach((log) => {
                    if (log.log.toLowerCase().includes(args[0].toString().toLowerCase())) {
                        listNames.push(`${c.character.name}: ${log.log}`);
                    }
                });
            }
            server.sendChatText(client, `Displaying list of players and logs matching criteria: ${listNames.join(",\n")}`, true);
        }
    },
    {
        name: "findloot",
        ...
        execute: (server, client, args) => {
            ...
            for (const a in server._lootableConstruction) {
                ...
                if (!(0, utils_1.isPosInRadius)(Number(args[1]), client.character.state.position, lootableConstrucion.state.position))
                    continue;
                let count = 0;
                ...
                if (count) {
                    server.sendChatText(client, `COUNT: ${count} POSITION: [ ${lootableConstrucion.state.position[0]} ${lootableConstrucion.state.position[1]} ${lootableConstrucion.state.position[2]} ]`);
                }
            }
    ...
    },
    {
        ...
        permissionLevel: types_1.PermissionLevels.DEFAULT,
        execute: async (server, client, args) => {
            const stats = server._gatewayServer.getSoeClientNetworkStats(client.soeClientId);
            if (stats) {
    ...
    },
    {
        ...
        execute: (server, client, args) => {
            ...
            server.sendChatText(client, `position: ${position[0].toFixed(2)},${position[1].toFixed(2)},${position[2].toFixed(2)}`, true);
    ...
    },
    {
        ...
        execute: async (server, client, args) => {
            ...
            const animationId = emotes_1.emoteMap[emoteName];
            if (!animationId) {
                server.sendChatText(client, `Unknown emote: ${args[0]}. Use /emotelist to see available emotes.`);
                return;
            }
            // Check if this emote is in the allowed list (no ownership check needed)
            if (emotes_1.defaultEmotes.includes(animationId)) {
                server.sendDataToAllWithSpawnedEntity(server._characters, client.character.characterId, "Animation.Play", {
            ...
            const accountItems = await server.accountInventoriesManager.getAccountItems(client.loginSessionId);
            for (const accountItem of accountItems) {
                if (accountItem && accountItem.itemDefinitionId) {
                    const itemDef = server.getItemDefinition(accountItem.itemDefinitionId);
    ...
    },
    {
        ...
        execute: async (server, client, args) => {
            ...
            for (const [emoteName, animationId] of Object.entries(emotes_1.emoteMap)) {
                if (emotes_1.defaultEmotes.includes(animationId)) {
            ...
            for (const accountItem of accountItems) {
                if (accountItem && accountItem.itemDefinitionId) {
                    ...
                    if (itemDef && itemDef.PARAM1) {
                        ...
                        for (const [emoteName, animationId] of Object.entries(emotes_1.emoteMap)) {
                            if (animationId === itemDef.PARAM1 &&
                                !emotes_1.defaultEmotes.includes(animationId)) {
                                ownedEmotes.push(emoteName);
                                break;
                            }
                        }
    ...
    },
    {
        ...
        execute: async (server, client, args) => {
            ...
            const targetClient = server.getClientByNameOrLoginSession(targetPlayerName);
            if (await server.chatManager.checkMute(server, client)) {
                server.sendChatText(client, "[Whisper] Message blocked, you are globally muted!");
                return;
            }
            if (targetClient instanceof zoneclient_1.ZoneClient2016) {
            ...
            server.sendChatText(client, `[Whisper to ${targetPlayerName}]: ${message}`);
            if (targetClient instanceof zoneclient_1.ZoneClient2016) {
    ...
    },
    {
        ...
        execute: async (server, client, args) => {
            ...
            const targetClient = server.getClientByNameOrLoginSession(targetPlayerName);
            if (await server.chatManager.checkMute(server, client)) {
                server.sendChatText(client, "[Reply] Message blocked, you are globally muted!");
                return;
            }
            if (targetClient instanceof zoneclient_1.ZoneClient2016) {
                if (targetClient.character?.characterId == client.character.characterId) {
                ...
                if (targetClient.character?.mutedCharacters?.includes(client.character.characterId)) {
                    server.sendChatText(client, `[Reply] Message blocked, target player has you muted!`);
                    return;
                }
                client.character.lastWhisperedPlayer = targetClient.character.name;
                targetClient.character.lastWhisperedPlayer = client.character.name;
            ...
            if (targetClient instanceof zoneclient_1.ZoneClient2016) {
                server.sendChatText(targetClient, `[Reply from ${client.character.name}]: ${message}`);
            }
        }
    },
    {
        ...
        execute: async (server, client, args) => {
            ...
            if (!targetClient || !(targetClient instanceof zoneclient_1.ZoneClient2016)) {
    ...
    },
    {
        ...
        execute: async (server, client, args) => {
            ...
            client.character.mutedCharacters.splice(client.character.mutedCharacters.indexOf(targetClient.character.characterId), 1);
    ...
    //#region MODERATOR PERMISSIONS
    {
        ...
        execute: async (server, client, args) => {
            ...
            for (const a in server._clients) {
                const c = server._clients[a];
                const clientStats = (await server._gatewayServer.getSoeClientNetworkStats(c.soeClientId)) ?? [];
    ...
    },
    {
        ...
        execute: async (server, client, args) => {
            ...
            const stats = await server._gatewayServer.getSoeClientNetworkStats(targetClient.soeClientId);
            if (stats) {
    ...
    },
    {
        ...
        execute: (server, client, args) => {
            ...
            if (client.character.isSpectator) {
                server.commandHandler.executeInternalCommand(server, client, "spectate", []);
            }
    ...
    },
    {
        ...
        execute: (server, client, args) => {
            ...
            if (index <= -1) {
                if (client.character._loadout[29] &&
                    client.character._loadout[29].itemDefinitionId == enums_1.Items.NV_GOGGLES) {
    ...
    },
    {
        ...
        execute: (server, client, args) => {
            ...
            switch (args[0]) {
                ...
                default:
                    if (args.length < 3) {
                        ...
                        server.sendChatText(client, "Set location list: farm, zimms, pv, br, ranchito, dam, cranberry, church, desoto, toxic, radiotower, villas, military, hospital, opfer, bubbas, crescent, dart, spence", false);
                        return;
                    }
                    const pos = client.character.state.position, x = args[0] == "~"
                        ? pos[0]
                        : Object.is(NaN, Number(args[0]))
                            ? pos[0]
                            : Number(args[0]), y = args[1] == "~"
                        ? pos[1]
                        : Object.is(NaN, Number(args[1]))
                            ? pos[1]
                            : Number(args[1]), z = args[2] == "~"
                        ? pos[2]
            ...
            const triggerLoadingScreen = !(0, utils_1.isPosInRadius)(250, client.character.state.position, position);
    ...
    },
    {
        ...
        execute: (server, client, args) => {
            ...
            targetClient.character.lastLoginDate = (0, utils_1.toHex)(Date.now());
            server.dropAllManagedObjects(targetClient);
            const triggerLoadingScreen = !(0, utils_1.isPosInRadius)(250, client.character.state.position, targetClient.character.state.position);
    ...
    },
    {
        ...
        execute: async (server, client, args) => {
            if (!args[0]) {
                server.sendChatText(client, `Correct usage: /ban {name} optional: {time (minutes)} {reason} {--silent}`);
                return;
            }
            ...
            const bannedClient = (await server._db
                ...
                .findOne({ name: args[0], active: true }));
            if (bannedClient) {
            ...
            const collection = server._db.collection(enums_2.DB_COLLECTIONS.CHARACTERS), character = (await collection.findOne({
                characterName: args[0],
                serverId: server._worldId,
                status: 1
            }));
            let ownerId = character?.ownerId, characterName = character?.characterName;
            if (!character) {
                const banClient = server.getClientByName(args[0]);
                if (server.playerNotFound(client, args[0].toString(), banClient)) {
                    return;
                }
                if (!banClient || !(banClient instanceof zoneclient_1.ZoneClient2016)) {
                    server.sendChatText(client, `Character with name ${args[0]} not found!`);
                    return;
                }
            ...
            if (!isNaN(time) && time > 0) {
                time += Date.now();
                server.sendChatText(client, `You have ${isSilent ? "silently " : ""}banned ${character?.characterName} until ${(0, utils_1.getDateString)(time)}`);
            }
            else {
                server.sendChatText(client, `You have ${isSilent ? "silently " : ""}banned ${character?.characterName} permanently`);
            }
            const reason = args.slice(2).join(" ");
            server.banClient(ownerId ?? "", characterName ?? "", reason, client.loginSessionId, time, isSilent);
        }
    },
    {
        name: "banid",
        ...
        execute: async (server, client, args) => {
            if (!args[0]) {
                server.sendChatText(client, `Correct usage: /banid {loginSessionId} optional: {time (minutes)} {reason} {--silent}`);
                return;
            }
            // prevent banning yourself
            ...
            const bannedClient = (await server._db
                ?.collection(enums_2.DB_COLLECTIONS.BANNED)
                .findOne({ loginSessionId: args[0], active: true }));
            if (bannedClient) {
    ...
    },
    {
        ...
        execute: async (server, client, args) => {
            ...
            if (unBannedClient) {
                ...
                server.sendChatText(client, `Cannot find any banned user with name ${name}`);
            }
        }
    },
    {
        ...
        execute: async (server, client, args) => {
            ...
            if (unBannedClient) {
                ...
                server.sendChatText(client, `Cannot find any banned user with id ${args[0]}`);
            }
        }
    },
    {
    ...
    },
    {
        ...
        execute: (server, client, args) => {
            ...
            if (isNaN(height)) {
                server.sendChatText(client, "Error: Please enter a valid number for the height.");
                return;
            }
            ...
            if (playerName) {
                ...
                if (!foundClient || !(foundClient instanceof zoneclient_1.ZoneClient2016)) {
                    server.sendChatText(client, "Client not found.");
                    return;
                }
                targetClient = foundClient;
            }
            const newPosition = new Float32Array(targetClient.character.state.position);
            switch (direction) {
                case "up":
                ...
                case "down":
                    ...
                    break;
                default:
                    server.sendChatText(client, "Error: Invalid direction. Use 'up' or 'down'.");
                    return;
            }
            server.sendData(targetClient, "ClientUpdate.UpdateLocation", {
            ...
            if (targetClient === client) {
                ...
                server.sendChatText(client, `Moved ${targetClient.character.name} ${direction} by ${height}`);
            }
        }
    },
    {
        ...
        execute: async (server, client, args) => {
            if (!args[0]) {
                server.sendChatText(client, `Correct usage: /listprocesses {name | ZoneClientId}`);
                return;
            }
            const targetClient = server.getClientByNameOrLoginSession(args[0].toString());
            if (server.playerNotFound(client, args[0].toString(), targetClient)) {
    ...
    },
    {
        ...
        execute: async (server, client, args) => {
            if (!args[0]) {
                server.sendChatText(client, `Correct usage: /listmodules {name | ZoneClientId}`);
                return;
            }
            const targetClient = server.getClientByNameOrLoginSession(args[0].toString());
            if (server.playerNotFound(client, args[0].toString(), targetClient)) {
            ...
            if (targetClient.isAdmin) {
                server.sendChatText(client, `${targetClient.character.name} is an admin!`);
            }
            server.sendChatText(client, `Requesting modules from: ${targetClient.character.name}`);
            server.sendData(targetClient, "H1emu.RequestModules", {});
            server.sendData(targetClient, "UpdateWeatherData", server.weatherManager.weather);
        }
    },
    {
        name: "listinfo",
        ...
        execute: async (server, client, args) => {
            if (!args[0]) {
                server.sendChatText(client, `Correct usage: /listinfo {name | ZoneClientId}`);
                return;
            }
            const targetClient = server.getClientByNameOrLoginSession(args[0].toString());
            if (server.playerNotFound(client, args[0].toString(), targetClient)) {
            ...
            server.sendChatText(client, `Requesting modules and windows from: ${targetClient.character.name}`);
            server.sendData(targetClient, "H1emu.RequestModules", {});
    ...
    },
    {
        ...
        execute: async (server, client, args) => {
            if (!args[0]) {
                server.sendChatText(client, `Correct usage: /listwindows {name | ZoneClientId}`);
                return;
            }
            const targetClient = server.getClientByNameOrLoginSession(args[0].toString());
            if (server.playerNotFound(client, args[0].toString(), targetClient)) {
            ...
            server.sendChatText(client, `Requesting windows from: ${targetClient.character.name}`);
            server.sendData(targetClient, "H1emu.RequestWindows", {});
            server.sendData(targetClient, "UpdateWeatherData", server.weatherManager.weather);
        }
    },
    {
        ...
        execute: async (server, client, args) => {
            if (!args[0]) {
                server.sendChatText(client, `Correct usage: /globalmute {name|playerId} optional: {time} {reason}`);
                return;
            }
            const targetClient = server.getClientByNameOrLoginSession(args[0].toString());
            if (server.playerNotFound(client, args[0].toString(), targetClient)) {
            ...
            const mutedClient = (await server._db
                ?.collection(enums_2.DB_COLLECTIONS.MUTED)
                .findOne({
            ...
            if (time > 0) {
                time += Date.now();
                server.sendChatText(client, `You have muted ${targetClient.character.name} until ${(0, utils_1.getDateString)(time)}`);
            }
            else {
                server.sendChatText(client, `You have muted ${targetClient.character.name} permanently`);
            }
            const reason = args.slice(2).join(" ");
            server.chatManager.muteClient(server, targetClient, reason, client.character.name ? client.character.name : "", time);
        }
    },
    {
        name: "globalunmute",
        ...
        execute: async (server, client, args) => {
            ...
            const name = args.join(" ").toString(), mutedClient = (await server._db?.collection(enums_2.DB_COLLECTIONS.MUTED).findOneAndUpdate({ name, active: true }, {
            ...
            if (mutedClient) {
                ...
                server.sendChatText(client, `Cannot find any muted user with name ${name}`);
            }
        }
    },
    {
    ...
    },
    {
        ...
        execute: (server, client, args) => {
            ...
            server.sendChatText(client, `Max packet loss set to ${packetloss} from ${server.maxPacketLoss}`, false);
    ...
    //#region ADMIN PERMISSIONS
    {
        ...
        execute: (server, client, args) => {
            ...
            if (typeof targetClient === "string") {
                server.sendChatText(client, `Player not found, did you mean ${targetClient}?`);
                return;
            }
            if (targetClient) {
    ...
    },
    {
        ...
        execute: async (server, client, args) => {
            ...
            server.sendDataToAllWithSpawnedEntity(server._characters, client.character.characterId, "Character.ReplaceBaseModel", {
                characterId: client.character.characterId,
                modelId: newModelId
            });
    ...
    },
    {
        ...
        execute: (server, client, args) => {
            client.spawnedEntities.forEach((object) => {
                if (object.characterId == client.character.characterId)
                    return;
    ...
    },
    {
        ...
        execute: (server, client, args) => {
            ...
            server.sendChatText(client, `Will force time to be ${server.inGameTimeManager.baseTimeMultiplier}x faster on next sync...`, true);
        }
    },
    {
        name: "freezetime",
    ...
    },
    {
        ...
        execute: (server, client, args) => {
            if (!args[1]) {
                server.sendChatText(client, "[ERROR] Usage /spamzombies [RANGE] [POINTS]");
                return;
            }
            ...
            points.forEach((obj) => {
                server.worldObjectManager.createNpc(server, 9634, new Float32Array([
                    obj[0],
        ...
        execute: (server, client, args) => {
            ...
            points.forEach((obj) => {
                ...
                server._explosives[characterId] = new explosiveentity_1.ExplosiveEntity(characterId, server.getTransientId(characterId), 9176, new Float32Array([
                    ...
                    obj[1],
                    1
                ]), client.character.state.lookAt, server, enums_1.Items.IED, client.character.characterId); // save explosive
            });
        }
    },
    {
    ...
    },
    {
        ...
        execute: (server, client, args) => {
            ...
            mimic.identity.characterName = args[0]
                .split("")
                .map((letter) => Math.random() < 0.7 || !/[a-z]/.test(letter)
                ? letter
                : letter.toUpperCase())
                .join("");
            ...
            for (const a in server._clients) {
                const c = server._clients[a];
                if ((0, utils_1.isPosInRadius)(c.character.npcRenderDistance || 250, client.character.state.position, c.character.state.position)) {
                    ...
                    server.sendData(c, "LightweightToFullPc", {
                        ...
                        stats: client.character.getStats().map((stat) => {
    ...
    },
    {
        ...
        execute: (server, client, args) => {
            if (!args[0] || !args[1]) {
                server.sendChatText(client, "[ERROR] Usage /desiredweather {setting} {value} optional: {alertAll: true/false}");
                return;
            }
            ...
            switch (args[0]) {
                case "skycolor":
                    if (value < -2 || value > 2) {
                        server.sendChatText(client, "Setting out of value range (MIN -2 - 2 MAX) (-2 = Deep blue to +2 = Deep Orange)");
                        return;
                    }
                ...
                case "globalprecipation":
                    if (value < 0 || value > 0.95) {
                        server.sendChatText(client, "Setting out of value range (MIN 0.0 - 0.95 MAX)");
                        return;
                    }
                ...
                case "fogdensity":
                    if (value < 0.0001 || value > 0.0002) {
                        server.sendChatText(client, "Setting out of value range (MIN 0.0001 - 0.0002 MAX)");
                        return;
                    }
                    changed = true;
                ...
                case "fogfloor":
                    if (value < 1 || value > 300) {
                        server.sendChatText(client, "Setting out of value range (MIN 1 - 300 MAX)");
                        return;
                    }
                    changed = true;
                ...
                case "foggradient":
                    if (value < 1 || value > 300) {
                        server.sendChatText(client, "Setting out of value range (MIN 0.001 - 0.02)");
                        return;
                    }
                    changed = true;
                ...
                default:
                    ...
                    server.sendChatText(client, "skycolor, globalprecipation, fogdensity, fogfloor, foggradient");
                    break;
            }
    ...
    },
    {
        ...
        permissionLevel: types_1.PermissionLevels.DEV,
        execute: (server, client, args) => {
            (0, node_fs_1.writeFileSync)(__dirname + "/../../../../../data/2016/sampleData/staticbuildings.json", JSON.stringify(server.staticBuildings));
        }
    },
    {
        ...
        execute: (server, client, args) => {
            if (!args[0]) {
                server.sendChatText(client, "[ERROR] Usage /addcontaineritem {itemDefinitionId/item name} optional: {count}");
                return;
            }
    ...
    },
    {
        ...
        execute: (server, client, args) => {
            if (!args.length) {
                server.sendChatText(client, "[ERROR] Usage /giverewardtoall {CrateID} [CrateID ...]");
                return;
            }
            ...
            for (const arg of args) {
                ...
                const validRewardItem = server.rewardManager.rewards.some((v) => v.itemId === rewardId);
                if (!validRewardItem) {
                    invalid.push(arg);
                    continue;
                }
            ...
            if (!rewardIds.length) {
                server.sendChatText(client, `[ERROR]${invalid.length ? " Crate ID: " + invalid.join(", ") : ""} is not valid`);
                return;
            }
            server.sendAlertToAll(`${client.character.name} has just initiated a crate drop`);
    ...
    },
    {
        ...
        execute: (server, client, args) => {
            if (!args[1]) {
                server.sendChatText(client, "[ERROR] Usage /givereward {itemDefinitionId} {playerName|playerId}");
                return;
            }
            const rewardId = Number(args[0]);
            const validRewardItem = server.rewardManager.rewards.some((v) => v.itemId === rewardId);
            if (!validRewardItem) {
                server.sendChatText(client, `[ERROR] ${rewardId} isn't a valid reward item`);
                return;
            }
            const targetClient = server.getClientByNameOrLoginSession(args[1].toString());
            if (typeof targetClient == "string") {
                server.sendChatText(client, `Could not find player ${args[1]
                    .toString()
            ...
            server.sendAlertToAll(`Admin ${client.character.name} rewarded ${targetClient.character.name} with ${enums_1.Items[rewardId]}`);
    ...
    },
    {
        ...
        execute: (server, client, args) => {
            if (!args[0]) {
                client.character.equipLoadout(server, loadouts_1.characterKitLoadout, true);
                return;
            }
            switch (args[0]) {
                case "pvp":
                ...
                case "parts":
                    ...
                    client.character.equipLoadout(server, loadouts_1.characterVehicleKit, true);
                    break;
                case "skins":
                    client.character.equipItem(server, server.generateItem(enums_1.Items.FANNY_PACK_DEV));
                    client.character.equipLoadout(server, loadouts_1.characterSkinsLoadout, true);
                    break;
                case "build":
                    client.character.equipItem(server, server.generateItem(enums_1.Items.FANNY_PACK_DEV));
                    client.character.equipLoadout(server, loadouts_1.characterBuildKitLoadout, true);
                    break;
                default:
    ...
    },
    {
        ...
        execute: (server, client, args) => {
            client.character.equipItem(server, server.generateItem(enums_1.Items.FANNY_PACK_DEV));
            client.character.equipLoadout(server, loadouts_1.characterTestKitLoadout, true);
    ...
    },
    {
        ...
        execute: (server, client, args) => {
            if (!args[0]) {
                server.sendChatText(client, `Correct usage: /vehiclerespawntimer <time>`);
                return;
            }
    ...
    },
    {
        ...
        execute: async (server, client, args) => {
            ...
            const characters = worlddatamanager_1.WorldDataManager.convertCharactersToSaveData(Object.values(server._characters), server._worldId);
    ...
    },
    {
        ...
        execute: async (server, client, args) => {
            ...
            server.sendChatText(client, `Next save at ${new Date(server.worldDataManager.nextSaveTime)}`);
        }
    },
    {
        name: "disablesave",
    ...
    },
    {
        ...
        execute: async (server, client, args) => {
            ...
            entitiesToDelete.forEach((entity) => {
                server.deleteEntity(entity.characterId, entity.dictionary, enums_1.Effects.PFX_Impact_Explosion_Landmine_Dirt_10m, 500);
            });
    ...
    },
    {
        ...
        execute: async (server, client, args) => {
            if (!args[0]) {
                server.sendChatText(client, `"[ERROR] Usage /unclaimbases {name / clientId}"`);
                return;
            }
            const targetClient = server.getClientByNameOrLoginSession(args[0].toString());
            if (server.playerNotFound(client, args[0].toString(), targetClient)) {
    ...
    },
    {
        ...
        execute: async (server, client, args) => {
            ...
            const time = Number(args[0]), message = args.slice(1, args.length).join(" ");
            if (isNaN(time)) {
