...
class ConfigManager {
    defaultConfig;
    config;
    constructor(server, configPath = `${process.cwd()}/config.yaml`) {
        this.defaultConfig = this.loadYaml("/../../../../data/2016/sampleData/defaultconfig.yaml");
        if (!configPath || !fileExists(configPath)) {
            ...
            (0, utils_1.copyFile)(`${__dirname}/../../../../data/2016/sampleData/defaultconfig.yaml`, `${process.cwd()}/config.yaml`);
    ...
    loadYaml(path, relative = true) {
        return yaml.load(fs.readFileSync(`${relative ? __dirname : ""}${path}`, "utf8"));
    }
    loadConfig(config) {
        // in case new config file is missing certain values / out of date,
        // fill with default values
        const { server, rcon, challenges: challenge, fairplay, voicechat, weather, airdrop, worldobjects, speedtree, construction, decay, smelting, randomevents, groups } = this.defaultConfig;
        return {
            ...this.defaultConfig,
            ...config,
            server: {
    ...
    applyConfig(server) {
        //#region server
        const { gameMode, proximityItemsDistance, interactionDistance, charactersRenderDistance, tickRate, worldRoutineRate, welcomeMessage, adminMessage, enableLoginServerKickRequests, rebootTime, rebootWarnTime, isPvE, isHeadshotOnly, isFirstPersonOnly, isNoBuildInPois, baseConstructionDamage, damageWeapons, disablePOIManager, disableMapBoundsCheck } = this.config.server;
        server.gameMode =
            enums_1.GameModes[gameMode.trim().toUpperCase()] ??
                enums_1.GameModes.SURVIVAL;
        ...
        //#region Challenges
        const { enabled: challengeEnabled, challengePerDay } = this.config.challenges;
        ...
        //
        //#region voicechat
        const { useVoiceChatV2, joinVoiceChatOnConnect, serverAccessToken, serverAddress } = this.config.voicechat;
        ...
        const { useFairplay, maxPing, acceptedRejectionTypes, useAssetValidation, hashSubmissionTimeout, allowedPacks, requiredPacks } = this.config.fairplay;
        ...
        //#endregion
        //#region airdrops
        const { planeMovementSpeed, crateDropSpeed, minimumPlayers } = this.config.airdrop;
        ...
        //#endregion
        //#region worldobjects
        const { vehicleSpawnCap, hasCustomLootRespawnTime, lootRespawnTimer, vehicleRespawnTimer, waterSourceRefillAmount, waterSourceReplenishTimer, npcRespawnTimer, itemDespawnTimer, lootDespawnTimer, deadNpcDespawnTimer, chanceWornLetter, vehicleSpawnRadius, npcSpawnRadius, chanceNpc, chanceScreamer, lootbagDespawnTimer, crowbarHitRewardChance, crowbarHitDamage } = this.config.worldobjects;
        ...
        //#endregion
        //#region construction
        const { allowStackedPlacement, allowOutOfBoundsPlacement, placementRange, spawnPointBlockedPlacementRange, vehicleSpawnPointBlockedPlacementRange, playerFoundationBlockedPlacementRange, playerShackBlockedPlacementRange, lowerStrongholdDefenses } = this.config.construction;
        ...
        //#endregion
        //#region decay
        const { decayTickInterval, constructionDamageTicks, ticksToFullDecay, worldFreeplaceDecayMultiplier, vehicleDamageTicks, vacantFoundationTicks, griefFoundationTimer, griefCheckSlotAmount, baseVehicleDamage, maxVehiclesPerArea, vehicleDamageRange, dailyRepairMaterials } = this.config.decay;
        ...
        //
        //#region gameTime
        const { timeFrozen, timeMultiplier, nightTimeMultiplier, baseTime } = this.config.gametime;
        ...
        //#endregion
        //#region groups
        const { enabled, player_limit, foundation_player_limit } = this.config.groups;
