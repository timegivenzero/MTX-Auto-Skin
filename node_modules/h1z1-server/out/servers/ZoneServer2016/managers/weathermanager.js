...
function rnd_number(max, min = 0, fixed = false) {
...
function getOrderedNumbersInRange(min, max, length) {
    if (length > max - min + 1) {
...
function adjustNumber(desiredNumber, currentNumber, changeNumber) {
...
class WeatherManager extends node_events_1.default {
    ...
    handleWeatherCommand(server, client, args) {
    ...
    async handleSaveCommand(server, client, args) {
        if (!args[0]) {
            server.sendChatText(client, "Please define a name for your weather template '/savecurrentweather {name}'");
        }
            ...
            if (server._soloMode) {
                ...
                node_fs_1.default.writeFileSync(`${__dirname}/../../../../data/2016/dataSources/weather.json`, JSON.stringify(this.templates, null, "\t"));
                ...
                this.templates = await server._db
                    .collection("weathers")
                    .find()
                    .toArray();
            }
    ...
    handleRandomCommand(server, client) {
        if (this.dynamicEnabled) {
            this.dynamicEnabled = false;
            server.sendChatText(client, "Dynamic weather removed !");
        }
    ...
    sendUpdateToAll(server, client, broadcast = false) {
        server.sendDataToAll("UpdateWeatherData", this.weather);
        if (client && broadcast) {
            server.sendGlobalChatText(`User "${client.character.name}" has changed weather.`);
        }
    }
    seasonstart() {
    ...
    moveToDesiredValues() {
        this.fogDensity = adjustNumber(this.desiredfogDensity, this.fogDensity, 0.0000025);
        this.fogFloor = adjustNumber(this.desiredfogFloor, this.fogFloor, 1);
        this.fogGradient = adjustNumber(this.desiredfogGradient, this.fogGradient, 0.00015);
    ...
    moveGPtoDesiredValue() {
        this.globalPrecipation = adjustNumber(this.desiredGlobalPrecipation, this.globalPrecipation, 0.012);
    }
    chooseWeather() {
        ...
        if (!this.lastDayWasFoggy && this.fogAllowed) {
            ...
            switch (fogType) {
                ...
                case 2:
                    ...
                    this.desiredfogGradient = 0.02;
                    if (this.desiredSkyColor < 0.5)
                        this.desiredSkyColor = 0.5;
                    break;
                case 3:
                    this.desiredfogDensity = rnd_number(0.0002, 0.00008);
